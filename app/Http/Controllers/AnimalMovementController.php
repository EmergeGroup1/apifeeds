...
class AnimalMovementController extends Controller
{
      ...
      **/
      private function filterTransferDayRemainingRemoved($data,$group_table,$group_bins_table)
      {
          ...
          $groups = $groups->whereBetween('created_at',[date("Y-m-d H:i:s",strtotime($data['date_from'])),date("Y-m-d",strtotime($data['date_to'] . " +1 day"))]);
          $groups = $groups->orderBy('date_to_transfer','desc');
          $groups = $groups->get();
          $groups = $this->toArray($groups);
      ...
      **/
      ...
      private function currentAge($start_date,$type,$group_id)
      {
        ...
        if($type == "nursery"){
          ...
          if($transfered_groups->where('group_to',$group_id)->get()->isNotEmpty()){
        ...
        if($gb->count() > 1){
          ...
          if($counter > 1){
            ...
            $total_current_pigs = DB::table("feeds_movement_groups_bins")
                                    ...
                                    ->sum("number_of_pigs");
            return ($total_pigs*100) / $total_current_pigs;                        
      ...
      **/
      public function createTransferAPIV2($data)
      {
          ...
          if($current_from_total_pigs_from == 0){
      ...
      */
      private function updateGroupsBinsPigsV2($transfer_bins,$unique_id,$transfer_type,$group_from_id,$group_to_id,$poor,$user_id)
      {
        ...
        if($transfer_type == 'farrowing_to_nursery'){
          ...
          // get the number_of_pigs for the bins in group to
          $number_of_pigs_to = DB::table('feeds_movement_groups_bins')->where('bin_id',$transfer_bins['bin_id_to'])->where('unique_id',$group_to_unique_id)->orderBy('id','desc')->first();
          $added_pigs = $number_of_pigs_to->number_of_pigs + $transfer_bins['number_of_pigs_transferred'];
          $orig_pigs = $number_of_pigs_to->orig_number_of_pigs + $transfer_bins['number_of_pigs_transferred'];
